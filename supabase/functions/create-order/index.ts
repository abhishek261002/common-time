...
if (!authHeader) {
  console.error("AUTH FAILED: No Authorization header provided");
  return new Response(
...
if (!authHeader.startsWith("Bearer ")) {
  console.error("AUTH FAILED: Authorization header does not start with 'Bearer '");
  return new Response(
    JSON.stringify({ error: "Invalid Authorization header format - must start with 'Bearer '" }),
    {
      status: 401,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    }
...
if (!token) {
  console.error("AUTH FAILED: Token is empty after removing Bearer prefix");
  return new Response(
...
if (authError) {
  console.error("AUTH FAILED: Supabase validation error:", authError);
  return new Response(
    JSON.stringify({ error: "Token validation failed: " + authError.message }),
    {
      status: 401,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    }
...
if (!data?.user) {
  console.error("AUTH FAILED: No user data returned from Supabase");
  return new Response(
    JSON.stringify({ error: "Invalid or expired token - no user found" }),
    {
      status: 401,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    }
...
console.log("AUTH SUCCESS: User validated -", data.user.id);
...
// In dev mode, return mock Razorpay order ID
